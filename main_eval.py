...
def eval_test(event_detector, model_type, val_errors, test_errors, Ytest, eval_metrics=['F1'], percentile=0.995, window=1, plot=False, best=False):
    ...
    for metric in eval_metrics:
        ...
        # Compute Accuracy
        acc = accuracy_score(Ytest_trunc, Yhat_trunc)
        print(f'At theta={used_theta}, window={used_window}, Accuracy={acc}')
...
def hyperparameter_eval(event_detector, model_type, config, val_errors, test_errors, Ytest,
    ...
    for metric in eval_metrics:
        ...
        # 👇 Add this compile line to silence warning and enable metrics
        try:
            event_detector.model.compile(optimizer="adam", loss="mse", metrics=["accuracy"])
        except AttributeError:
            # some event_detector wrappers may not expose .model directly
            pass
