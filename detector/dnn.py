...
class DeepNN(ICSDetector):
    ...
    def create_model(self):
        """ Creates Keras DNN model. """

        # retrieve params
        nI = self.params['nI']  # number of inputs (features)
        ...
        if layers < 1:
            print(f'Error: Must have at least one layer. Found layers={layers}')
            return

        # Input expects (history, nI)
        input_layer = Input(shape=(self.params['history'], nI))

        # Flatten (history, nI) → (history*nI,)
        x = Flatten()(input_layer)

        # First hidden layer
        deep_layer = Dense(units, activation=activation)(x)

        # Additional hidden layers
        for _ in range(layers - 1):
            deep_layer = Dense(units, activation=activation)(deep_layer)

        # Output layer (reconstruction of input features)
    ...
    def reconstruction_errors(self, x, batches=False, eval_batch_size=4096, **keras_params):
        if batches:
            full_errors = np.zeros((x.shape[0], x.shape[1]))

            idx = 0
            while idx < len(x):
                Xwindow, Ywindow = self.transform_to_window_data(
                    x[idx: idx + eval_batch_size],
                    x[idx: idx + eval_batch_size]
                )

                # Reshape Xwindow → (batch, history, nI)
                Xwindow = Xwindow.reshape((Xwindow.shape[0], self.params['history'], self.params['nI']))

                preds = self.predict(Xwindow, **keras_params)

                # Ensure preds and Ywindow shapes match
                if preds.ndim > 2:
                    preds = preds.reshape(preds.shape[0], preds.shape[-1])

                batch_errors = (preds - Ywindow) ** 2

                # Fill errors into the right slice
                end_idx = min(idx + eval_batch_size, len(full_errors))
                rows_to_copy = min(full_errors[idx:end_idx].shape[0], batch_errors.shape[0])
                full_errors[idx:idx + rows_to_copy] = batch_errors[:rows_to_copy]
