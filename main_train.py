    ...
    for metric in eval_metrics:
        ...
        for percentile_idx in range(len(cutoffs)):
            ...
            for window_idx in range(len(windows)):
                ...
                # Ensure predictions and ground truth align
                if len(Yhat) > len(Ytest_val):
                    Yhat = Yhat[:len(Ytest_val)]
                elif len(Yhat) < len(Ytest_val):
                    Ytest_val = Ytest_val[:len(Yhat)]
        ...
        # For non-AE models, ensure alignment with the clipped test data
        if not model_type == 'AE':
            history = event_detector.params['history']
            # Clip final_Yhat to match the length of Ytest_test
            final_Yhat = final_Yhat[history + 1:]

        # Ensure both arrays have the same length
        min_len = min(len(final_Yhat), len(Ytest_test))
        final_Yhat = final_Yhat[:min_len].astype(int)
        Ytest_test = Ytest_test[:min_len]

        metric_func = metrics.get(metric)
        final_value = metric_func(final_Yhat, Ytest_test)
        acc=accuracy(final_Yhat, Ytest_test)
        print("Final {} is {:.3f} at percentile={:.5f}, window {}, Accuracy {}".format(metric, final_value, best_percentile, best_window,acc))
...
if __name__ == "__main__":
    ...
    if model_type == 'AE' or model_type == 'ID':  # Add ID here
        ...
        else:  # ID model - create directly without training data
            # Set the input size parameter manually for Identity
            config['model']['nI'] = Xfull.shape[1]
            event_detector = identity.Identity(**config['model'])
            event_detector.create_model()

        # For validation data in hyperparameter search
        if model_type == 'AE':
            validation_data = Xval
        else:  # ID model
            validation_data = Xfull  # Use full data since Identity doesn't actually train

        # Search for the best tuning of the window and theta parameters
        hyperparameter_search(event_detector, model_type, config, validation_data, Xtest, Ytest, dataset_name,
        ...
        history = config['model'].get('history', 1)  # default to 1 for non-sequence models
